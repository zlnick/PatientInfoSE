...
def prepare_table():
    try:
        ...
        clear_sql = """Drop table DrugInfo.Insurance"""
        cursor.execute(clear_sql)
        table_sql = """
            CREATE TABLE IF NOT EXISTS DrugInfo.Insurance (
                DrugClass VARCHAR(32),
                DrugNumber VARCHAR(32),
                DrugName VARCHAR(1024),
                NameEmbedding VECTOR(FLOAT,1536),
                DrugForm VARCHAR(1024),
...
def insert_data_by_row(df):
    ...
    sql_insert = """
        INSERT INTO DrugInfo.Insurance (DrugClass, DrugNumber, DrugName, NameEmbedding, DrugForm, RuleInsurance)
        VALUES (?, ?, ?, ?, ?, ?)
    """
    cursor = connection.cursor()
    # 逐行打印数据
    for index, row in df.iterrows():
        ...
        print(f"第 {row_number} 行数据：")
        drugClass = row['药品分类']
        drugNumber = row['编号']
        drugName = row['药品名称']
        nameEmbedding = get_embedding(drugName)
        embedding_str = ",".join(map(str, nameEmbedding))
        drugForm = row['剂型']
        ruleInsurance = row['备注']
        args = [drugClass,drugNumber,drugName,embedding_str,drugForm,ruleInsurance]
